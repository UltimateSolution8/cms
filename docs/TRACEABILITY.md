...
| **s.5(1)** + **Rule 3** | The notice accompanying or preceding a consent request states the data and purpose, the manner of exercising s.6(4) and s.13 rights, and the manner of complaining to the Board | Notice served with an integrity hash and version; three specific Rule 3(c) links, not a contact page | `NoticeIT` — *"the current notice renders with the three URIs Rule 3 requires"*; `ReceiptIT` — *"the receipt carries the Rule 3 links and the notice it was given against"* | satisfied — the mechanism and all three limbs are present |
| **s.5(3)**, **s.6(3)** | The principal may access the notice, and the consent request, in English or an Eighth Schedule language | `notice_translation` per language, coverage reported and placeholder rows refused | `NoticeIT`; `GET /v1/notices/reports/coverage` | **partial** — **19 of 23 languages have no text.** Procurement, not code. **The language duty is in the Act (s.5(3), s.6(3)), not in Rule 3** — Rule 3's own text carries no language clause, and this row said otherwise until Phase 17 |
| **s.6(3)** + **Rule 9** | The consent request carries DPO or authorised-person contact details; those details are also published standingly on the site or app and repeated in every rights response | `dpoContact` and `grievanceUri` on the receipt, **resolved up the entity hierarchy**; `EntityContactCheck` names any entity resolving to null at start-up | `ReceiptIT`; `EntityContactCheck` | **UDS decision** — the mechanism is complete and `dpo_contact` on the `UDS` row is unset, so every receipt names a null contact. One statement fixes all fifteen entities. **The DPO-contact duty is s.6(3) and Rule 9, not Rule 3** — corrected in Phase 17 |
| **s.5(1)(ii)** + **Rule 3(c)** | The notice states the *manner* of withdrawing, as one of the rights it must explain how to exercise | `withdrawalUri` on every receipt, read off the notice version served | `ReceiptIT`; `NoticeIT` | satisfied |
...
| **s.6(6)** | On withdrawal the fiduciary shall, within a reasonable time, **cease and cause its Data Processors to cease** processing | The engine ceases immediately — a withdrawal denies the next decision before any sweeper runs. The *cause-its-processors-to-cease* limb is the outbox chain plus the propagation register that names them. **Previously folded into the s.6(5) row and never carried under its own number** | `ConsentLifecycleIT`; `PropagationIT` | **configuration** — same register, same §8.7 signature |
| **DPDP — an Art. 19 analogue** | **There is none, and that is the finding.** s.6(6) and s.8(7)(b) run *fiduciary → its own processors*; neither the Act nor the Rules 2025 requires the fiduciary to *demonstrate* that cessation reached the processor, to notify third-party recipients generally, or to tell the principal who they were | Nothing, deliberately. The propagation evidence above **exceeds** what DPDP asks and is built for GDPR Art. 19 and Art. 5(2). DPDP is stronger in one respect — no disproportionate-effort carve-out — and weaker in two | — | **not an obligation** — recorded so nobody later reads the webhook evidence as a DPDP requirement, or removes it believing DPDP compels it |
| s.8(7) | Erase on withdrawal or when the purpose is no longer served | `RetentionSweeper` **proposes** erasure and never deletes evidence; the acts happen in named systems | `RetentionIT` — *"a dormant subject is proposed for erasure, and their evidence is untouched"* | **partial** — proposal and evidence are the platform's; the erasure **act** is a `fulfilment_target` (§8.5), and the **instruction** to a processor is §8.7's propagation register. Two registers, deliberately: one obligation split across both is how neither gets filled |
...
| — | The same, for **admin intake** (`POST /v1/rights`, roles CAPTURE or ADMIN) | The caller still supplies `receivedAt` — a request arriving by post or over the phone is real, and the clock runs from the principal's act — but it is now **bounded in both directions and its provenance is recorded**. Future beyond `ClockTolerance.SKEW` is refused (it would move the deadline outward); older than `uds.consent.rights.max-backdate` (90d) is refused as a **sanity bound** — not, as this row said for three phases, because it would file a request already in breach: every computed period is shorter than 90 days, so a late filing inside the bound is accepted and *is* overdue, which is correct, and the audit event records `bornOverdue` to say so; and `rights_request.verification_method` records `PORTAL_TOKEN` / `OPERATOR_ASSERTED` / `UNVERIFIED`, on the row and on the append-only audit event | `RightsRequestIT` — *"a receivedAt in the future is refused, because it would move the deadline outward"*, *"a receivedAt beyond the backdate bound is refused, as a sanity check on the value"*, *"a late filing is accepted, and the audit row says it arrived overdue"*, *"both start-instant refusals reach the caller as 400, not as a 500"*, *"a request nobody verified says so, rather than looking like every other one"*, *"a row written without the column reads UNVERIFIED, never as though it were checked"* | **partial** — the bound and the label are real; the *verification itself* on this path is still an operator's claim rather than something the platform establishes, and `UNVERIFIED` is deliberately not a gate. What an audit can now answer is which requests rest on what, which it could not before |
...
| Art. 7(3) | Withdrawal as easy as giving, **and effective** | Withdrawal path plus **propagation evidence**: outbox → `WebhookPublisher` (HMAC over the exact bytes sent) → a `webhook_delivery` row per attempt, and another entity's subscriber receives nothing | `WebhookDeliveryIT` — *"a consent event is pushed to the subscriber and the delivery is recorded"*, *"the payload is signed over the exact bytes sent"* | **configuration** — the mechanism is complete and better than anything in the field (`competitive-analysis.md` §B.2). **Phase 17 changed what the gap is.** The platform can now *name* the systems that must be told (`propagation_target`) and report those nothing can reach, so an unregistered system no longer leaves no trace. It stays *configuration* because the register is empty until UDS signs §8.7 — and because propagation is only **evidenced** on the webhook publisher; under the default `log` publisher the platform records `NO_DELIVERY_CHANNEL` rather than inferring non-delivery |
...
| **Art. 19**, limb 1 | Communicate any erasure or restriction of processing **to each recipient** to whom the data were disclosed, unless impossible or disproportionate | The outbox → `WebhookPublisher` → `webhook_delivery` chain, which was already limb 1's mechanism and was cited only under Art. 7(3). **`propagation_target` is what makes *"each recipient"* enumerable**: mandatory targets with no active subscription are reported as uncovered, and `propagation_gap` records daily what could not be shown to have been met | `PropagationIT` — *"an unreachable target is uncovered, and registering the subscription clears it"*, *"a system nobody registered is recorded as NO_SUBSCRIPTION, against the subject"*, *"a FAILED delivery does not satisfy the obligation"* | **configuration** — the mechanism and the register exist; the register is empty until §8.7 is signed |
| **Art. 19**, limb 2 | Inform the data subject about those recipients **if they request it** | Largely discharged already and above the statutory floor: `VendorStore.vendorsForPurpose` → `ReceiptService` names active recipients per purpose on **every** receipt, proactively rather than on request. The evidence bundle's `propagation` section adds the narrower thing a receipt cannot say — *who was told about **this** withdrawal, and who was not* — summarised per system rather than per message | `ReceiptIT`; `EvidenceBundleIT` — *"the bundle is complete and stays complete"* | **partial** — the receipt's proactive recipient list is genuinely above the statutory floor. The bundle's `propagation` section is **not** fully per-principal: `systemUnmetDays` is register-level, because `propagation_gap` is deduplicated per day without the subject, and `webhook_delivery.subject_id` begins at `V31` so earlier deliveries are unattributed. Both are declared on the record rather than left for a reader to assume |
| **Art. 5(2)**, **Art. 24(1)** | Be able to **demonstrate** compliance | The evidence plane generally, and for propagation specifically `webhook_delivery` and `propagation_gap`, both append-only. **A register that is empty demonstrates nothing**, because nothing was sent — which is the limb §8.7 exists to close | `LedgerAppendOnlyIT`; `PropagationIT` — *"two relay passes over one message leave one gap row, not two"* | **configuration** |
...
| Art. 15 | Access — a copy of the data and the processing information | `GET /v1/admin/evidence/subject/{entityId}/{subjectId}`; resolves merges (every section reads across the merged id set), enumerates subject-scoped tables from `information_schema` so a new table fails the build, **assembling somebody's whole file is itself recorded**, and the bundle now **declares its own incompleteness** — receipts cap at 100 and denials at 200, and a `truncation` entry names the section, the cap and a ready-to-run request that returns the remainder. **The cap is applied per subject id**, so a merged principal — the one most likely to exceed it — gets a pointer naming the id that actually holds the overflow rather than the canonical id, which could not deliver it | `EvidenceBundleIT` — *"a bundle that could not fit everything says so, and says where the rest is"*, *"a bundle that fits carries no truncation notice at all"*, *"assembling somebody's whole file is itself recorded"*, *"the evidence bundle returns the whole person, and names what was merged in"* (`SubjectIdentityIT`) | **partial** — the silent-truncation limb is closed. What remains is **entity scope**: a Board question about a *person* is fifteen calls under fifteen scopes. Deliberately not fixed with a group-level route, which would have to bypass `EntityAccessGuard` and the RLS claim at once; the SOP is `OPERATIONS.md` §12.2a, and an SOP is weaker than a route and is the correct trade here |
...
6. **The platform decides nothing a person should decide.** `OPERATIONS.md` §10 is the list of what it
   deliberately does not decide; this matrix is not a licence to read a permission out of it.
7. **Four DPDP citations in this matrix were wrong, and were corrected in Phase 17 against primary
   text.** Ease-of-withdrawal was cited as **s.6(6)**; it is **s.6(4)**, and s.6(6) is the separate
   duty to *"cease and cause its Data Processors to cease processing"*. The notice row cited
   **s.6(6) + Rule 3** and was wrong on both axes: the notice obligation is **s.5(1)**, the language
   option is **s.5(3) and s.6(3)** — in the Act, **not** in Rule 3, whose own text has no language
   clause — and the DPO-contact duty is **s.6(3)** at request time plus **Rule 9** as a standing
   publication duty, not Rule 3. The same conflation sat in six javadocs, in
   `/regulatory-clause-map`, and in `REGULATORY_HANDOFF.md`; a third-party review then reproduced it
   out of `DpdpModule`'s comment rather than catching it.

   **Source quality, stated rather than implied.** The Act sections are cross-checked on two
   independent transcriptions that agree word-for-word (indiankanoon.org and dpdpa.com, accessed
   18 August 2026). **The Rules are not held to that standard**: MeitY's Gazette PDF returns HTTP
   403, and the one full-text mirror that downloaded could not be rendered, so Rules 3 and 9 rest on
   two rule-mirror sites that agree with each other. That is weaker evidence than the Act rows and
   is recorded as such, because the alternative is a matrix that looks uniformly authoritative and
   is not.
