...
|---|---|---|---|
| s.6(6), Rule 3 | Notice carries the required elements, in the required languages; principal is given DPO contact and a grievance route | Notice served with integrity hash; receipt carries `dpo_contact` and `grievance_uri`, resolved up the entity hierarchy | `DpdpModule` ~l.56, `ReceiptService`, `EntityContactCheck` |
